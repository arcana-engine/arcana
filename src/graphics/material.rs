...
impl PartialEq for Material {
    fn eq(&self, other: &Self) -> bool {
        if self.albedo_coverage != other.albedo_coverage {
            return false;
        }
        ...
        if OrderedFloat(self.coverage_factor) != OrderedFloat(other.coverage_factor) {
            return false;
        }
...
impl Hash for Material {
    ...
    where
        H: Hasher,
    {
        self.albedo_coverage.hash(state);
        ...
        OrderedFloat(self.coverage_factor).hash(state);
...
mod defaults {
    ...
    pub const fn coverage_factor() -> f32 {
        1.0
    }
